...
def splitPrefix(unitstring):
    '''Detects if the unit is in fractional form.'''
    if '/' in unitstring: #If the division symbol is present in the string,
                          #indicating a numerator and denominator
        unitstringsplit = unitstring.split('/') #Split the string in half along
                                                #the '/'
    elif 'per' in unitstring.lower(): #Otherwise, if there is a 'per' present in
                                      #the strong (1 meter per second)
        unitstringsplit = unitstring.split('per') #Split the string in half
                                                  #along the 'per'
    else: #Otherwise, pass.
        pass
    return unitstringsplit #Return unitstringsplit as a list

def detectPrefix(unitstringsplit):
    '''Detects the prefixes.'''
    print ("The passed string is %s") % (unitstringsplit) #TEST: What string was
                                                          #passed?
    unitmultiplier = 1 #Default multiplier is 1.
    multiplierlist = [] #<>
    pattern = re.compile("^((?:\d*\.\d+)|\d+)(?:\^((?:\d*\.\d+)|\d+))?") #Regex
    match = pattern.match(unitstringsplit) #Check unitstringsplit for the regex
                                           #pattern
    if match is not None: #If a match is found
        ...
        if exp == None: #If there's no exponential
            ...
            scalarmultiplier = num ** exp #The scalar multiplier is the number
                                          #raised to the exponential, ie 13^2
                                          #would be num == 13, exp == 2,
                                          #scalarmultiplier == 13 ** 2
                                          #prefixlist =
                                          #{'milli':.001,'Milli':.001,'m'
    prefixlist = {'centi':.01,'Centi':.01,'c':.01} #Sample dictionary of
                                                   #prefixes to check for in the
                                                   #1/100s range
    ...
    for prefixkey in prefixlist.keys(): #For each prefixkey in the list
        if prefixkey in unitstringsplit and prefixkey[-1] != unitstringsplit[-1]:
            #Check if it's in unitstringsplit AND if the last prefixkey does not
            #match the last unitstringsplit.  This is to make sure that if, say,
            #you have a mm^3 value, the second m doesn't trigger the dictionary.
            unitmultiplier *= prefixlist.get(prefixkey) #Multiply the unit
                                                        #multiplier by the
                                                        #return - ie c is .01
            ...
            #else:
                #multiplierlist.append(unitmultiplier)\
    #Print out the scalar multiplier and unit multiplier.
    print ("Scalar multiplier is %r. Unit multiplier is %r") % (scalarmultiplier, unitmultiplier)
    return float(scalarmultiplier)*unitmultiplier #Return the scalar multiplier
                                                  #times the unit multiplier
